...
def get_params(params):
    ...
    parser.add_argument(
        "--encoding_method_name",
        type=str,
        default="one_hot",
        help="Specifies the encoding method for the dataset. Can be 'one_hot' or 'float_range'.",
    )
...
def train(opts, datasets, verbose_callbacks=False):
    ...
    if opts.early_stopping:
        callbacks.extend(
            [
                InteractionSaverEarlyStopping(
                    [opts.n_epochs],
                    test_epochs=[opts.n_epochs],
                    checkpoint_dir=opts.save_path,
                ),
                EarlyStopperLossWithPatience(
                    patience=opts.patience, min_delta=opts.min_delta
                ),
            ]
        )
    ...
    if len(test):
        ...
        if opts.test_rsa:
            ...
            rsa_game = core.SenderReceiverRnnGS(
                rsa_sender, receiver, loss, length_cost=opts.length_cost
            )
            ...
            with torch.no_grad():
                ...
                if opts.save:
                    ...
                    pickle.dump(
                        loss_and_metrics,
                        open(opts.save_path + "/loss_and_metrics.pkl", "wb"),
                    )
    ...
    for run in range(opts.num_of_runs):
        ...
        if not opts.load_dataset and not opts.zero_shot:
            data_set = dataset.DataSet(
                ...
                encoding_method_name=opts.encoding_method_name,
            )
        ...
        if not opts.zero_shot:
            ...
            if opts.load_checkpoint:
                opts.checkpoint_path = os.path.join(
                    opts.game_path, str(run), "final.tar"
                )
            ...
            if opts.load_interaction:
                opts.interaction_path = os.path.join(
                    opts.game_path,
                    str(run),
                    "interactions",
                    opts.test_rsa,
        ...
        if opts.test_rsa:
            if (
                opts.test_rsa == "train"
                or opts.test_rsa == "validation"
                or opts.test_rsa == "test"
            ):
                # create subfolder if necessary
                opts.save_path = os.path.join(
                    opts.game_path, str(run), "interactions", opts.test_rsa
                )
                if not os.path.exists(opts.save_path) and opts.save:
                    os.makedirs(opts.save_path)
        ...
        if opts.zero_shot:
            ...
            if opts.zero_shot_test is not None:
                # create subfolder if necessary
                opts.save_path = os.path.join(
                    opts.game_path, "zero_shot", opts.zero_shot_test
                )
                if not os.path.exists(opts.save_path) and opts.save:
                    os.makedirs(opts.save_path)
            ...
            if opts.load_checkpoint:
                opts.checkpoint_path = os.path.join(
                    opts.save_path, str(run), "final.tar"
                )
                if not os.path.exists(opts.checkpoint_path):
                    raise ValueError(
                        f"Checkpoint file {opts.checkpoint_path} not found."
